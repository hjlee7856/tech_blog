...
function extractCategories(recordMap: ExtendedRecordMap): string[] {
  ...
  const categories = Object.values(recordMap.block)
    .filter((b: any) => b.value?.type === 'page' && b.value?.parent_table === 'collection')
    .map((b: any) => {
...
export function useCategory({ recordMap }: UseCategoryProps): UseCategoryReturn {
...
export function filterRecordMapByCategory(
  ...
  for (const [id, blockData] of Object.entries(filteredMap.block)) {
    ...
    if (block?.type === 'page' && block?.parent_table === 'collection') {
      const blockCategory = (block.properties as Record<string, Decoration[]> | undefined)?.[
        categoryKey
      ]?.[0]?.[0];

      if (blockCategory === category || block.id === currentPageId || block.id === rootPageId) {
